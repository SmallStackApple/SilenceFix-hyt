...
public class BlinkUtils
{
    ...
    static {
        BLACKLIST.add(C00Handshake.class);
        BLACKLIST.add(C00PacketLoginStart.class);
        BLACKLIST.add(C00PacketServerQuery.class);
        BLACKLIST.add(C01PacketChatMessage.class);
        BLACKLIST.add(C01PacketEncryptionResponse.class);
        BLACKLIST.add(C01PacketPing.class);
    }
    ...
    public static void releasePacket(final String packetType, final boolean onlySelected, final int amount, final int minBuff) {
        ...
        if (packetType == null) {
            ...
            for (final Packet<INetHandlerPlayServer> packets : BlinkUtils.playerBuffer) {
                final int packetID = getPacketId(packets);
    ...
    public static boolean pushPacket(final Packet<?> packets) {
        final int packetID = getPacketId(packets);
        if (packetID <= BlinkUtils.packetToggleStat.length && !Minecraft.getMinecraft().isSingleplayer() && BlinkUtils.packetToggleStat[packetID] && !isBlacklisted(packets)) {
    ...
    public void startBlink(EventPacketSend event, Packet<?> packetIn) {
    ...
    private static boolean isBlacklisted(Packet<?> packet) {
        return BLACKLIST.contains(packet.getClass());
    }
    ...
    private static int getPacketId(Packet<?> packet) {
        ...
        Integer packetId = connectionState.getPacketId(EnumPacketDirection.SERVERBOUND, packet);

        if (packetId == null) {
            packetId = connectionState.getPacketId(EnumPacketDirection.CLIENTBOUND, packet);
        }
