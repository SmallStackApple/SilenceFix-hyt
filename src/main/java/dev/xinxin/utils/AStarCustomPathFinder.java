...
public class AStarCustomPathFinder {
        ...
        block0: for (int i = 0; i < loops; ++i) {
            ...
            for (Hub hub : new ArrayList<Hub>(this.hubsToWork)) {
                ...
                for (Vec3 direction : flatCardinalDirections) {
                    Vec3 loc = BlockUtil.floorVec3(hub.getLoc().add(direction));
                    if (AStarCustomPathFinder.checkPositionValidity(loc, false) && this.addHub(hub, loc, 0.0)) break block0;
                }
                Vec3 loc1 = BlockUtil.floorVec3(hub.getLoc().addVector(0.0, 1.0, 0.0));
                if ((!AStarCustomPathFinder.checkPositionValidity(loc1, false) || !this.addHub(hub, loc1, 0.0)) && (!AStarCustomPathFinder.checkPositionValidity(loc2 = BlockUtil.floorVec3(hub.getLoc().addVector(0.0, -1.0, 0.0)), false) || !this.addHub(hub, loc2, 0.0))) continue;
                break block0;
            }
        }
    ...
    public static boolean checkPositionValidity(int x2, int y2, int z, boolean checkGround) {
        BlockPos block1 = new BlockPos(x2, y2, z);
        BlockPos block2 = new BlockPos(x2, y2 + 1, z);
        BlockPos block3 = new BlockPos(x2, y2 - 1, z);
