...
public class ProjectileUtil {
    public static ProjectileHit predict(double posX, double posY, double posZ, double motionX, double motionY, double motionZ, double motionSlowdown, double size, double gravity, boolean draw) {
        ...
        while (!hasLanded && posY > -60.0) {
            ...
            landingPosition = SilenceFix.mc.theWorld.rayTraceBlocks(posBefore, posAfter, false, true, false);
            ...
            List<Entity> entityList = SilenceFix.mc.theWorld.getEntitiesWithinAABB(Entity.class, arrowBox.addCoord(motionX, motionY, motionZ).expand(1.0, 1.0, 1.0));
            for (int i = 0; i < entityList.size(); ++i) {
                AxisAlignedBB var2;
                MovingObjectPosition possibleEntityLanding;
                Entity var18 = entityList.get(i);
                if (!var18.canBeCollidedWith() || var18 == SilenceFix.mc.thePlayer || (possibleEntityLanding = (var2 = var18.getEntityBoundingBox().expand(size, size, size)).calculateIntercept(posBefore, posAfter)) == null) continue;
                hitEntity = true;
                hasLanded = true;
                landingPosition = possibleEntityLanding;
            }
            BlockPos var35 = new BlockPos(posX += motionX, posY += motionY, posZ += motionZ);
            Block var36 = SilenceFix.mc.theWorld.getBlockState(var35).getBlock();
            ...
            motionY -= gravity;
            if (!draw) continue;
            GL11.glVertex3d((double)(posX - SilenceFix.mc.getRenderManager().getRenderPosX()), (double)(posY - SilenceFix.mc.getRenderManager().getRenderPosY()), (double)(posZ - SilenceFix.mc.getRenderManager().getRenderPosZ()));
        }
    ...
    public static class EnderPearlPredictor {
        ...
        private double assessSingleRotation(Vector2f rotation, double motionYOffset) {
            ...
            double posY = this.predictY + (double) SilenceFix.mc.thePlayer.getEyeHeight() - (double)0.1f;
            ...
            ProjectileHit projectileHit = ProjectileUtil.predict(posX, posY, posZ, motionX *= motionFactor, motionY += motionYOffset, motionZ *= motionFactor, 0.99f, 0.25, 0.03f, false);
            if (projectileHit.hasLanded && projectileHit.landingPosition != null) {
                BlockPos landingPos = projectileHit.landingPosition.getBlockPos();
                BlockPos belowLandingPos = landingPos.down();
                Block blockBelow = mc.theWorld.getBlockState(belowLandingPos).getBlock();
        ...
        private double assessSingleBlockPos(BlockPos pos) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int z = -1; z <= 1; z++) {
                    if (x == 0 && z == 0) continue;
                    if (isUnsafePosition(pos.add(x, 0, z))) {
                        safetyScore *= 0.7; // 周围有不安全区域则降低评分
                    }
                }
            }
            int solidBelow = 0;
            for (int y = -1; y >= -5; y--) {
                IBlockState state = mc.theWorld.getBlockState(pos.add(0, y, 0));
                if (state.getBlock().isFullBlock()) {
                    solidBelow++;
                }
            }
            safetyScore *= Math.min(1.0, solidBelow / 3.0); // 下方支撑越多评分越高
        ...
        private boolean isUnsafePosition(BlockPos pos) {
            ...
            if (state.getBlock().getMaterial() == Material.air) {
                for (int y = -1; y >= -3; y--) {
                    if (!mc.theWorld.isAirBlock(pos.add(0, y, 0))) {
                        return false;
                    }
                }
                return true;
