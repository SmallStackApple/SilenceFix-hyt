...
public final class SlotUtil {
    ...
    public static int findBlock() {
        for (int i = 36; i < 45; ++i) {
            Block block;
            ItemStack item = SilenceFix.mc.thePlayer.inventoryContainer.getSlot(i).getStack();
    ...
    public static void setSlot(int slot, boolean render) {
        ...
        SilenceFix.mc.thePlayer.inventory.alternativeCurrentItem = slot;
        SilenceFix.mc.thePlayer.inventory.alternativeSlot = true;
    }

    public static ItemStack getItemStack() {
        return SilenceFix.mc.thePlayer == null || SilenceFix.mc.thePlayer.inventoryContainer == null ? null : SilenceFix.mc.thePlayer.inventoryContainer.getSlot(SlotUtil.getItemIndex() + 36).getStack();
    }

    public static int getItemIndex() {
        InventoryPlayer inventoryPlayer = SilenceFix.mc.thePlayer.inventory;
    ...
    public static int findSword() {
        ...
        int bestSlot = -1;
        for (int i = 0; i < 9; ++i) {
            ItemStack itemStack = SilenceFix.mc.thePlayer.inventory.getStackInSlot(i);
    ...
    public static int findTool(BlockPos blockPos) {
        ...
        int bestSlot = -1;
        IBlockState blockState = SilenceFix.mc.theWorld.getBlockState(blockPos);
        for (int i = 0; i < 9; ++i) {
            float speed;
            ItemStack itemStack = SilenceFix.mc.thePlayer.inventory.getStackInSlot(i);
    ...
    public static ItemStack getCurrentItemInSlot(int slot) {
        return slot < 9 && slot >= 0 ? SilenceFix.mc.thePlayer.inventory.mainInventory[slot] : null;
    }

    public static float getStrVsBlock(Block blockIn, int slot) {
        float f = 1.0f;
        if (SilenceFix.mc.thePlayer.inventory.mainInventory[slot] != null) {
            f *= SilenceFix.mc.thePlayer.inventory.mainInventory[slot].getStrVsBlock(blockIn);
        }
        return f;
    }

    public static float getPlayerRelativeBlockHardness(EntityPlayer playerIn, World worldIn, BlockPos pos, int slot) {
        Block block = SilenceFix.mc.theWorld.getBlockState(pos).getBlock();
    ...
    public static boolean canHeldItemHarvest(Block blockIn, int slot) {
        ...
        ItemStack itemstack = SilenceFix.mc.thePlayer.inventory.getStackInSlot(slot);
    ...
    public static float getToolDigEfficiency(Block blockIn, int slot) {
        ...
        if (f > 1.0f) {
            int i = EnchantmentHelper.getEfficiencyModifier(SilenceFix.mc.thePlayer);
            ItemStack itemstack = SlotUtil.getCurrentItemInSlot(slot);
            if (i > 0 && itemstack != null) {
                f += (float)(i * i + 1);
            }
        }
        if (SilenceFix.mc.thePlayer.isPotionActive(Potion.digSpeed)) {
            f *= 1.0f + (float)(SilenceFix.mc.thePlayer.getActivePotionEffect(Potion.digSpeed).getAmplifier() + 1) * 0.2f;
        }
        if (SilenceFix.mc.thePlayer.isPotionActive(Potion.digSlowdown)) {
            float f1;
            switch (SilenceFix.mc.thePlayer.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) {
                case 0: {
                    f1 = 0.3f;
                    break;
                }
        ...
        if (SilenceFix.mc.thePlayer.isInsideOfMaterial(Material.water) && !EnchantmentHelper.getAquaAffinityModifier(SilenceFix.mc.thePlayer)) {
            f /= 5.0f;
        }
        if (!SilenceFix.mc.thePlayer.onGround) {
            f /= 5.0f;
        }
        return f;
    }
