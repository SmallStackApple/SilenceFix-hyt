...
public class MoveUtil {
    ...
    public static double speed() {
        return Math.hypot(SilenceFix.mc.thePlayer.motionX, SilenceFix.mc.thePlayer.motionZ);
    }
    public static double speedCalculator() {
        if (mc.thePlayer != null) {
            EntityPlayer player = mc.thePlayer;
            double deltaX = player.posX - player.prevPosX;
            double deltaZ = player.posZ - player.prevPosZ;
            double bps = Math.hypot(deltaX, deltaZ) * mc.timer.timerSpeed * 20;
    ...
    public static boolean isBlockNearBy(double distance) {
        ...
        int x2 = (int)smallX;
        while ((double)x2 <= bigX) {
            int y2 = (int)smallY;
            while ((double)y2 <= bigY) {
                int z = (int)smallZ;
                while ((double)z <= bigZ) {
                    if (!MoveUtil.isPositionValidity(new Vec3(x2, y2, z)) && MoveUtil.isPositionValidity(new Vec3(x2, y2 + 1, z))) {
                        return true;
                    }
                    ++z;
                }
    ...
    public static float[] getRotationsBlock(BlockPos block, EnumFacing face) {
        double x2 = (double)block.getX() + 0.5 - MoveUtil.mc.thePlayer.posX + (double)face.getFrontOffsetX() / 2.0;
        double z = (double)block.getZ() + 0.5 - MoveUtil.mc.thePlayer.posZ + (double)face.getFrontOffsetZ() / 2.0;
        double y2 = (double)block.getY() + 0.5;
        double d1 = MoveUtil.mc.thePlayer.posY + (double)MoveUtil.mc.thePlayer.getEyeHeight() - y2;
    ...
    public static Block getBlockAtPosC(double x2, double y2, double z) {
        EntityPlayerSP inPlayer = MoveUtil.mc.thePlayer;
        return MoveUtil.mc.theWorld.getBlockState(new BlockPos(inPlayer.posX + x2, inPlayer.posY + y2, inPlayer.posZ + z)).getBlock();
    }
    ...
    public static boolean checkTeleport(double x2, double y2, double z, double distBetweenPackets) {
        double distx = MoveUtil.mc.thePlayer.posX - x2;
        double disty = MoveUtil.mc.thePlayer.posY - y2;
        double distz = MoveUtil.mc.thePlayer.posZ - z;
        double dist = Math.sqrt(MoveUtil.mc.thePlayer.getDistanceSq(x2, y2, z));
        ...
        while ((double)i < nbPackets) {
            AxisAlignedBB bb;
            double xdi = (x2 - MoveUtil.mc.thePlayer.posX) / nbPackets;
            double ydi = (y2 - MoveUtil.mc.thePlayer.posY) / nbPackets;
