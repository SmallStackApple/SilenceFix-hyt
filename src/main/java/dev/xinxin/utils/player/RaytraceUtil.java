...
public class RaytraceUtil {
    ...
    public static MovingObjectPosition rayCast(final RotationNew rotation, final double range, final float expand, Entity entity, boolean throughWall, float predict, float predictPlayer) {
        ...
        if (entity != null && mc.theWorld != null) {
            objectMouseOver = entity.rayTrace(range, rotation.yaw, rotation.pitch, predictPlayer);
            double d1 = range;
            ...
            final Vec3 vec31 = mc.thePlayer.getVectorForRotation(rotation.pitch, rotation.yaw);
            ...
            for (final Entity entity1 : list) {
                ...
                if (predict2 != 0) {
                    axisalignedbb = axisalignedbb.offset(
                            (target.posX - target.prevPosX) * predict2,
                            (target.posY - target.prevPosY) * predict2,
                            (target.posZ - target.prevPosZ) * predict2
                    );
                }
    ...
    public static MovingObjectPosition rayCast(Vector2f rotation, double range, float expand, boolean throughWall) {
        return RaytraceUtil.rayCast(rotation, range, expand, RaytraceUtil.mc.thePlayer, throughWall);
    }

    public static Entity raycastEntity(double range, float yaw, float pitch, IEntityFilter entityFilter) {
        Entity renderViewEntity = mc.getRenderViewEntity();
        if (renderViewEntity != null && RaytraceUtil.mc.theWorld != null) {
            ...
            List<Entity> entityList = RaytraceUtil.mc.theWorld.getEntitiesInAABBexcluding(renderViewEntity, renderViewEntity.getEntityBoundingBox().addCoord(entityLook.xCoord * blockReachDistance, entityLook.yCoord * blockReachDistance, entityLook.zCoord * blockReachDistance).expand(1.0, 1.0, 1.0), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            Entity pointedEntity = null;
    ...
    public static MovingObjectPosition rayCast(Vector2f rotation, double range, float expand, Entity entity, boolean throughWall) {
        ...
        if (entity != null && RaytraceUtil.mc.theWorld != null) {
            ...
            Vec3 vec31 = RaytraceUtil.mc.thePlayer.getVectorForRotation(rotation.y, rotation.x);
            Vec3 vec32 = vec3.addVector(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range);
            Entity pointedEntity = null;
            Vec3 vec33 = null;
            float f = 1.0f;
            List<Entity> list = RaytraceUtil.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range).expand(1.0, 1.0, 1.0), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            double d2 = d1;
    ...
    public static boolean overBlock(Vector2f rotation, EnumFacing enumFacing, BlockPos pos, boolean strict) {
        MovingObjectPosition movingObjectPosition = RaytraceUtil.mc.thePlayer.rayTraceCustom(4.5, rotation.x, rotation.y);
    ...
    public static boolean overBlock(EnumFacing enumFacing, BlockPos pos, boolean strict) {
        MovingObjectPosition movingObjectPosition = RaytraceUtil.mc.objectMouseOver;
        if (movingObjectPosition == null) {
            return false;
        }
        Vec3 hitVec = movingObjectPosition.hitVec;
    ...
    public static Boolean overBlock(Vector2f rotation, BlockPos pos) {
        return RaytraceUtil.overBlock(rotation, EnumFacing.UP, pos, false);
    }

    public static Boolean overBlock(Vector2f rotation, BlockPos pos, EnumFacing enumFacing) {
        return RaytraceUtil.overBlock(rotation, enumFacing, pos, true);
    }
    ...
    public static MovingObjectPosition rayCast(Vector2f rotation, double range, float expand) {
        ...
        if (entity != null && RaytraceUtil.mc.theWorld != null) {
            MovingObjectPosition objectMouseOver = entity.customRayTrace(range, partialTicks, rotation.getX(), rotation.getY());
            double d1 = range;
            Vec3 vec3 = entity.getPositionEyes(partialTicks);
            if (objectMouseOver != null) {
                d1 = objectMouseOver.hitVec.distanceTo(vec3);
            }
            Vec3 vec31 = RaytraceUtil.mc.thePlayer.getVectorForRotation(rotation.getY(), rotation.getX());
            Vec3 vec32 = vec3.addVector(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range);
            Entity pointedEntity = null;
            Vec3 vec33 = null;
            float f = 1.0f;
