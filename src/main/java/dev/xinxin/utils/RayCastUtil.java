...
public final class RayCastUtil {
    ...
    public static MovingObjectPosition rayCast(Vector2f rotation, double range, float expand) {
        return RayCastUtil.rayCast(rotation, range, expand, SilenceFix.mc.thePlayer);
    }
    ...
    public static MovingObjectPosition rayCast(Vector2f rotation, double range, float expand, Entity entity) {
        float partialTicks = SilenceFix.mc.timer.renderPartialTicks;
        if (entity != null && SilenceFix.mc.theWorld != null) {
            ...
            Vec3 vec31 = SilenceFix.mc.thePlayer.getVectorForRotation(rotation.y, rotation.x);
            Vec3 vec32 = vec3.addVector(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range);
            Entity pointedEntity = null;
            Vec3 vec33 = null;
            float f = 1.0f;
            List<Entity> list = SilenceFix.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range).expand(1.0, 1.0, 1.0), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            double d2 = d1;
    ...
    public static boolean overBlock(Vector2f rotation, EnumFacing enumFacing, BlockPos pos, boolean strict) {
        MovingObjectPosition movingObjectPosition = mc.thePlayer.rayTraceCustom(4.5, rotation.x, rotation.y);
    ...
    public static boolean overBlock(EnumFacing enumFacing, BlockPos pos, boolean strict) {
        MovingObjectPosition movingObjectPosition = SilenceFix.mc.objectMouseOver;
        if (movingObjectPosition == null) {
            return false;
        }
        Vec3 hitVec = movingObjectPosition.hitVec;
    ...
    public static MovingObjectPosition rayCast(float partialTicks) {
        ...
        Entity entity = SilenceFix.mc.getRenderViewEntity();
        if (entity != null && SilenceFix.mc.theWorld != null) {
            SilenceFix.mc.mcProfiler.startSection("pick");
            SilenceFix.mc.pointedEntity = null;
            double d0 = SilenceFix.mc.playerController.getBlockReachDistance();
            objectMouseOver = entity.rayTrace(d0, partialTicks);
            double d2 = d0;
            Vec3 vec3 = entity.getPositionEyes(partialTicks);
            boolean flag = false;
            boolean flag2 = true;
            if (SilenceFix.mc.playerController.extendedReach()) {
                d0 = 6.0;
                d2 = 6.0;
            } else if (d0 > 3.0) {
                flag = true;
            ...
            Vec3 vec6 = null;
            float f = 1.0f;
            List<Entity> list = SilenceFix.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec4.xCoord * d0, vec4.yCoord * d0, vec4.zCoord * d0).expand(1.0, 1.0, 1.0), (Predicate<? super Entity>)Predicates.and((Predicate[])new Predicate[]{EntitySelectors.NOT_SPECTATING}));
            double d3 = d2;
            Object realBB = null;
