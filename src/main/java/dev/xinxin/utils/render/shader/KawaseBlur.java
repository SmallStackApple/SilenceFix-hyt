...
public class KawaseBlur {
    ...
    private static void initFramebuffers(float iterations) {
        ...
        while ((float)i <= iterations) {
            Framebuffer currentBuffer = new Framebuffer((int)((double) SilenceFix.mc.displayWidth / Math.pow(2.0, i)), (int)((double) SilenceFix.mc.displayHeight / Math.pow(2.0, i)), false);
    ...
    public static void renderBlur111(int stencilFrameBufferTexture, int iterations, int offset) {
        int i;
        if (currentIterations != iterations || KawaseBlur.framebuffer.framebufferWidth != SilenceFix.mc.displayWidth || KawaseBlur.framebuffer.framebufferHeight != SilenceFix.mc.displayHeight) {
            KawaseBlur.initFramebuffers(iterations);
            currentIterations = iterations;
        }
        KawaseBlur.renderFBO(framebufferList.get(1), SilenceFix.mc.getFramebuffer().framebufferTexture, kawaseDown, offset);
        ...
        kawaseUp.setUniformi("textureToCheck", 17);
