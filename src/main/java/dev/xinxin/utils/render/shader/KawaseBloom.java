...
import static dev.xinxin.gui.ui.modules.ShaderElement.createFrameBuffer;
...
public class KawaseBloom {
    ...
    private static void initFramebuffers(float iterations) {
        ...
        while ((float)i <= iterations) {
            Framebuffer currentBuffer = new Framebuffer((int)((double) SilenceFix.mc.displayWidth / Math.pow(2.0, i)), (int)((double) SilenceFix.mc.displayHeight / Math.pow(2.0, i)), true);
    ...
    public static void shadow(Runnable drawMod, int iterations, int offset) {
        ...
        drawMod.run();
        stencilFramebuffer.unbindFramebuffer();
        KawaseBlur.renderBlur111(stencilFramebuffer.framebufferTexture, iterations, offset);
    }

    public static void renderBlur(int framebufferTexture, int iterations, int offset) {
        int i;
        if (currentIterations != iterations || KawaseBloom.framebuffer.framebufferWidth != SilenceFix.mc.displayWidth || KawaseBloom.framebuffer.framebufferHeight != SilenceFix.mc.displayHeight) {
