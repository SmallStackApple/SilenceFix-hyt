...
import dev.xinxin.utils.client.FileUtil;
...
public class ShaderUtil {
    ...
    private final String blurShader = "#version 120\n\n" +
            ...
            "        sum += texture2D(inTexture, uv + vec2(x * texelSize.x, 0.0)) * (1.0 - abs(x) / radius);\n" +
            "    }\n" +
            "    for (float y = -radius; y <= radius; y++) {\n" +
            "        sum += texture2D(inTexture, uv + vec2(0.0, y * texelSize.y)) * (1.0 - abs(y) / radius);\n" +
            "    }\n" +
            "    gl_FragColor = sum / (2.0 * radius + 1.0);\n" +
            "}\n";
    public ShaderUtil(String fragmentShaderLoc, String vertexShaderLoc) {
        ...
        try {
            ...
            switch (fragmentShaderLoc) {
                case "blurShader": {
                    fragmentShaderID = this.createShader(new ByteArrayInputStream(this.blurShader.getBytes()), 35632);
                    break;
                }
                case "kawaseUpGlow": {
                ...
                default: {
                    fragmentShaderID = this.createShader(SilenceFix.mc.getResourceManager().getResource(new ResourceLocation(fragmentShaderLoc)).getInputStream(), 35632);
                }
            }
            GL20.glAttachShader((int)program, (int)fragmentShaderID);
            int vertexShaderID = this.createShader(SilenceFix.mc.getResourceManager().getResource(new ResourceLocation(vertexShaderLoc)).getInputStream(), 35633);
    ...
    public ShaderUtil(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = this.createShader(SilenceFix.mc.getResourceManager().getResource(new ResourceLocation("express/shader/vertex.vsh")).getInputStream(), 35633);
        }
        catch (IOException e) {
    ...
    public static void drawQuads() {
        ScaledResolution sr = new ScaledResolution(SilenceFix.mc);
    ...
    private int createShader(InputStream inputStream, int shaderType) {
        ...
        GL20.glShaderSource((int)shader, (CharSequence)FileUtil.readInputStream(inputStream));
