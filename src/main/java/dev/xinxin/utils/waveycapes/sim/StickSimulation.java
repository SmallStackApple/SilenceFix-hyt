package dev.xinxin.utils.waveycapes.sim;

import dev.xinxin.utils.waveycapes.util.Mth;
...
public class StickSimulation {
    ...
    public void simulate() {
        ...
        final StickSimulation.Vector2 down = new StickSimulation.Vector2(0.0f, this.gravity * deltaTime);
        final StickSimulation.Vector2 tmp = new StickSimulation.Vector2(0.0f, 0.0f);
        for (final StickSimulation.Point p : this.points) {
            if (!p.locked) {
                tmp.copy(p.position);
        ...
        final StickSimulation.Point basePoint = this.points.get(0);
        for (final StickSimulation.Point p2 : this.points) {
        ...
        for (int i = this.points.size() - 2; i >= 1; --i) {
            ...
            if (abs < 180.0f - this.maxBend) {
                final StickSimulation.Vector2 replacement = this.getReplacement(this.points.get(i).position, this.points.get(i - 1).position, angle, 180.0f - this.maxBend + 1.0f);
        ...
        for (int i = 0; i < this.numIterations; ++i) {
            for (int x = this.sticks.size() - 1; x >= 0; --x) {
                final StickSimulation.Stick stick = this.sticks.get(x);
                final StickSimulation.Vector2 stickCentre = stick.pointA.position.clone().add(stick.pointB.position).div(2.0f);
                final StickSimulation.Vector2 stickDir = stick.pointA.position.clone().subtract(stick.pointB.position).normalize();
        ...
        for (int x2 = 0; x2 < this.sticks.size(); ++x2) {
            final StickSimulation.Stick stick2 = this.sticks.get(x2);
            final StickSimulation.Vector2 stickDir2 = stick2.pointA.position.clone().subtract(stick2.pointB.position).normalize();
