...
public class PacketUtil {
    ...
    public static void send(Packet<?> packet) {
        if (SilenceFix.mc.thePlayer != null) {
            SilenceFix.mc.getNetHandler().addToSendQueue(packet);
        }
    }
    ...
    private static byte[] encode() {
        try {
            ...
            gzipOutputStream.write("{\"packet_sub_type\":\"814:469\",\"packet_data\":\"2.6.10\",\"packet_type\":\"ver\"}".getBytes());
    ...
    public static void sendNoC0F() {
        sendPacketNoEvent(new C0FPacketConfirmTransaction(MathUtil.getRandom(114514, 191981000), (short)MathUtil.getRandomInRange(114514, 191981000), true));
    }
    ...
    public static void handlePacket(Packet<INetHandlerPlayClient> packet) {
        NetHandlerPlayClient netHandler = SilenceFix.mc.getNetHandler();
    ...
    public static boolean isFlying(EventPacketSend type) {
    ...
    public static boolean isFlyingOnlyC03(EventPacketSend type) {
        return type.getPacket() instanceof C03PacketPlayer || !(type.getPacket() instanceof C03PacketPlayer.C04PacketPlayerPosition) || !(type.getPacket() instanceof C03PacketPlayer.C05PacketPlayerLook) || !(type.getPacket() instanceof C03PacketPlayer.C06PacketPlayerPosLook);
    }
    ...
    public void sendC0F(int windowId, short uid, boolean accepted) {
        sendC0F(windowId, uid, accepted, false);
    }
    ...
    public static void sendPacketNoEvent(Packet<?> packet) {
        SilenceFix.mc.getNetHandler().addToSendQueueUnregistered(packet);
    }

    public static void receivePacketNoEvent(Packet<?> packet) {
        SilenceFix.mc.getNetHandler().addToReceiveQueueUnregistered(packet);
    }
