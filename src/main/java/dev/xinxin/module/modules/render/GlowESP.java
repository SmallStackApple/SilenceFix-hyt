...
import dev.xinxin.event.rendering.EventRenderModel;
...
extends Module {
    ...
    private final ModeValue<cmode> colorMode = new ModeValue("Color Mode", cmode.values(), cmode.Sync);
    ...
    private final NumberValue radius = new NumberValue("Radius", 4.0, 2.0, 20.0, 2.0);
    private final NumberValue iterationsSetting = new NumberValue("Iterations", 4.0, 2.0, 10.0, 1.0);
    private final NumberValue offsetSetting = new NumberValue("Offset", 4.0, 2.0, 10.0, 1.0);
    private final NumberValue exposure = new NumberValue("Exposure", 2.2, 0.5, 3.5, 0.1);
    ...
    public GlowESP() {
        super("GlowESP", Category.Render,"辉光透视");
        INSTANCE = this;
    }
    ...
    public void onRenderModelEvent(EventRenderModel e) {
        if (!e.isPost() && this.outlineValue.getValue().booleanValue()) {
    ...
    public void onRender2DEvent(EventRender2D e) {
        ...
        if (this.framebuffer != null && this.outlineFrameBuffer != null && this.entities.size() > 0) {
            ...
            if (this.kawaseGlow.getValue().booleanValue()) {
                int i;
                int offset = this.offsetSetting.getValue().intValue();
                int iterations = 3;
                ...
                for (i = 1; i < iterations; ++i) {
                    GlowESP.renderFBO(framebufferList.get(i + 1), GlowESP.framebufferList.get(i).framebufferTexture, this.kawaseGlowShader, offset);
                }
                for (i = iterations; i > 1; --i) {
                    GlowESP.renderFBO(framebufferList.get(i - 1), GlowESP.framebufferList.get(i).framebufferTexture, this.kawaseGlowShader2, offset);
                }
                ...
                this.kawaseGlowShader2.setUniformi("check", this.seperate.getValue() ? 1 : 0);
                this.kawaseGlowShader2.setUniformf("lastPass", 1.0f);
                this.kawaseGlowShader2.setUniformf("exposure", (float)((double) this.exposure.getValue().floatValue() * fadeIn.getOutput()));
                ...
                if (this.seperate.getValue().booleanValue()) {
                    GL13.glActiveTexture(34000);
                    RenderUtil.bindTexture(this.framebuffer.framebufferTexture);
                }
                GL13.glActiveTexture(33984);
    ...
    public void setupGlowUniforms(float dir1, float dir2) {
        ...
        this.glowShader.setUniformf("exposure", (float)((double) this.exposure.getValue().floatValue() * fadeIn.getOutput()));
        this.glowShader.setUniformi("avoidTexture", this.seperate.getValue() ? 1 : 0);
        FloatBuffer buffer = BufferUtils.createFloatBuffer(256);
        int i = 1;
        while ((float)i <= this.radius.getValue().floatValue()) {
            buffer.put(MathUtil.calculateGaussianValue(i, this.radius.getValue().floatValue() / 2.0f));
            ++i;
        }
        buffer.rewind();
    ...
    public void setupOutlineUniforms(float dir1, float dir2) {
        ...
        float iterations = this.kawaseGlow.getValue() ? this.iterationsSetting.getValue().floatValue() * 2.0f : this.radius.getValue().floatValue() / 1.5f;
