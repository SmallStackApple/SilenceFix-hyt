 ...
 * create 29/12/2023
 */
public class Packet79 implements GermPacket {
    private String name;
    private int i;
    private String data;
    ...
    public void process() {
        ...
        if (prefix.equals("updateOption")) {
            String[] part = data.split("<->")[1].split("@");
            String type = part[1];
            String json = part[2];
            if (type.equals("variables")) {
                ...
                for (JsonElement element : object.getAsJsonArray("subs")) {
                    ...
                    List<String> desc = Arrays.stream(gameObj.get("desc").getAsString()
                                    .replace("&", "§")
                                    .replace("✦","☆")
                                    .split("\n"))
            ...
            String[] part = data.split("<->")[1].split("_");
            if (part[0].equals("team")) {
                ...
                packet.setMessage3(data.split("<->")[1]);
                packet.setN2(1);
    ...
    public void readPacketData(PacketBuffer packetBuffer) {
        ...
        data = packetBuffer.readStringFromBuffer(3276700);
    }
