        ...
        if (horse.isChested()) {
            for (int k2 = 0; k2 < i; ++k2) {
                for (int l2 = 0; l2 < 5; ++l2) {
                    this.addSlotToContainer(new Slot(horseInventoryIn, 2 + l2 + k2 * 5, 80 + l2 * 18, 18 + k2 * 18));
                }
            }
        }
        for (int i1 = 0; i1 < 3; ++i1) {
            for (int k1 = 0; k1 < 9; ++k1) {
                this.addSlotToContainer(new Slot(playerInventory, k1 + i1 * 9 + 9, 8 + k1 * 18, 102 + i1 * 18 + j2));
            }
        }
        for (int j1 = 0; j1 < 9; ++j1) {
            this.addSlotToContainer(new Slot(playerInventory, j1, 8 + j1 * 18, 160 + j2));
        }
    }
