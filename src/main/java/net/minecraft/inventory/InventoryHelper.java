...
public class InventoryHelper {
    ...
    private static void dropInventoryItems(World worldIn, double x2, double y2, double z, IInventory inventory) {
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ItemStack itemstack = inventory.getStackInSlot(i);
            if (itemstack == null) continue;
            InventoryHelper.spawnItemStack(worldIn, x2, y2, z, itemstack);
        }
    }

    private static void spawnItemStack(World worldIn, double x2, double y2, double z, ItemStack stack) {
        ...
        while (stack.stackSize > 0) {
            ...
            stack.stackSize -= i;
            EntityItem entityitem = new EntityItem(worldIn, x2 + (double)f, y2 + (double)f1, z + (double)f2, new ItemStack(stack.getItem(), i, stack.getMetadata()));
