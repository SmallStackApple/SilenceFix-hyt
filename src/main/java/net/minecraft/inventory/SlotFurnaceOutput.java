    ...
    protected void onCrafting(ItemStack stack) {
        ...
        if (!this.thePlayer.worldObj.isRemote) {
            ...
            if (f == 0.0f) {
                ...
                int j2 = MathHelper.floor_float((float)i * f);
                if (j2 < MathHelper.ceiling_float_int((float)i * f) && Math.random() < (double)((float)i * f - (float)j2)) {
                    ++j2;
                }
                i = j2;
            }
            while (i > 0) {
                int k2 = EntityXPOrb.getXPSplit(i);
                i -= k2;
