...
import com.xinxin.client.viaversion.vialoadingbase.netty.event.CompressionReorderEvent;
import com.xinxin.client.viaversion.viamcp.MCPVLBPipeline;
...
import dev.xinxin.utils.component.Class19;
...
public class NetworkManager
    ...
    public static final LazyLoadBase<NioEventLoopGroup> CLIENT_NIO_EVENTLOOP = new LazyLoadBase<NioEventLoopGroup>() {
        ...
        protected NioEventLoopGroup load() {
            return new NioEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty SilenceFix IO #%d").setDaemon(true).build());
        }
    };
    public static final LazyLoadBase<EpollEventLoopGroup> CLIENT_EPOLL_EVENTLOOP = new LazyLoadBase<EpollEventLoopGroup>() {
        ...
        protected EpollEventLoopGroup load() {
            return new EpollEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Epoll SilenceFix IO #%d").setDaemon(true).build());
        }
    };
    public static final LazyLoadBase<LocalEventLoopGroup> CLIENT_LOCAL_EVENTLOOP = new LazyLoadBase<>() {
        ...
        protected LocalEventLoopGroup load() {
            return new LocalEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Local SilenceFix IO #%d").setDaemon(true).build());
        }
    };
    ...
    public void sendPacket(Packet packetIn) {
        EventHigherPacketSend higherEvent = new EventHigherPacketSend(packetIn);
        EventManager.call(higherEvent);
        if (higherEvent.isCancelled()) {
            return;
        }
        if ((packetIn instanceof C03PacketPlayer.C04PacketPlayerPosition || packetIn instanceof C03PacketPlayer.C05PacketPlayerLook || packetIn instanceof C03PacketPlayer.C06PacketPlayerPosLook) && Minecraft.getMinecraft().theWorld != null && Minecraft.getMinecraft().thePlayer != null) {
            ...
            if (!(packetIn instanceof C03PacketPlayer.C05PacketPlayerLook)) {
                Minecraft.getMinecraft().thePlayer.setLastServerPosition(Minecraft.getMinecraft().thePlayer.getServerPosition());
                Minecraft.getMinecraft().thePlayer.setServerPosition(new Vec3(c03PacketPlayer.getPositionX(), c03PacketPlayer.getPositionY(), c03PacketPlayer.getPositionZ()));
            }
        }
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        ...
        GetC03StatusUtil.packetEvent(inPacket);
        boolean blinkUtilCancel = Class19.Method9(inPacket);
        if (inPacket instanceof C07PacketPlayerDigging dig && dig.getStatus() == C07PacketPlayerDigging.Action.RELEASE_USE_ITEM) {
        }
        if (!blinkUtilCancel) {
            return;
        }
...
/*
        ...
        EventHigherPacketSend higherPacketSend = new EventHigherPacketSend(packetIn);
        SilenceFix.eventManager.call(higherPacketSend);
