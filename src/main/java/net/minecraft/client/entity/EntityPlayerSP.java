...
public class EntityPlayerSP
    ...
    public void updateTool(BlockPos pos) {
        ...
        if (slot != -1 && this.mc.thePlayer.inventory.getStackInSlot(this.inventory.currentItem) != this.inventory.getStackInSlot(slot)) {
            this.inventory.currentItem = slot;
            SilenceFix.instance.slotSpoofManager.startSpoofing(slot);
        }
    }
    ...
    public void onUpdateWalkingPlayer() {
        ...
        EventMotion PRE = new EventMotion(posX, posY, posZ, this.rotationYaw, this.rotationPitch, onGround, EventMotion.EventState.PRE);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            FastEat fastEat = SilenceFix.instance.moduleManager.getModule(FastEat.class);
            ...
            if (this.ridingEntity == null) {
                ...
                PRE.setEventState(EventMotion.EventState.POST);
                EventManager.call(PRE);
            } else {
...
//            while (!this.mc.scheduledTasks.isEmpty() && Disabler.shouldProcess()) {
//                try {
...
//    }
    ...
    public void setServerPosition(Vec3 severPosition) {
        this.serverPosition = severPosition;
    }
    ...
    public void sendChatMessage(String message) {
        ...
        if (message.startsWith(SilenceFix.instance.commandPrefix)) {
            String[] args = message.trim().substring(1).split(" ");
            Command c = SilenceFix.instance.commandManager.getCommand(args[0]);
            if (c != null) {
    ...
    public void onLivingUpdate() {
        ...
        EventSilentMove eventSilentMove = new EventSilentMove(SilenceFix.instance.yawPitchHelper.realYaw);
         ...
         */
        if ((this.isUsingItem() || AutoGapple.eating) && !this.isRiding() && !slowdown.isCancelled()) {
    ...
    public float[] mySilentStrafe(float strafe, float forward, float yaw, boolean advanced) {
        ...
        double[] realMotion = MoveUtil.getMotion(0.22, strafe, forward, SilenceFix.instance.getYawPitchHelper().realYaw);
    ...
    public ResourceLocation getLocationCape() {
        switch (HUD.capeMode.get()) {
            case MODE1 -> {
                return SilenceFix.capes[0];
            }
            case MODE2 -> {
                return SilenceFix.capes[1];
            }
            case MODE3 -> {
                return SilenceFix.capes[2];
            }
            case danzai -> {
                return SilenceFix.capes[3];
            }
            case danzai1 -> {
                return SilenceFix.capes[4];
            }
        }
