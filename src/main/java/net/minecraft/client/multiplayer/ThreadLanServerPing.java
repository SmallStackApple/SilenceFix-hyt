...
extends Thread {
    ...
    public static String getMotdFromPingResponse(String p_77524_0_) {
        ...
        int j2 = p_77524_0_.indexOf("[/MOTD]", i + "[MOTD]".length());
        return j2 < i ? "missing no" : p_77524_0_.substring(i + "[MOTD]".length(), j2);
    }
    ...
    public static String getAdFromPingResponse(String p_77523_0_) {
        ...
        int j2 = p_77523_0_.indexOf("[/MOTD]", i + "[/MOTD]".length());
        if (j2 >= 0) {
            return null;
        }
        int k2 = p_77523_0_.indexOf("[AD]", i + "[/MOTD]".length());
        if (k2 < 0) {
            return null;
        }
