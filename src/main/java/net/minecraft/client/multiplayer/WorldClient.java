...
extends World {
    ...
    public int worldTicks = 0;
    public boolean hasQueriedPlayerBed = false;
    ...
    public void removeEntity(Entity entityIn) {
        ...
        if (entityIn instanceof EntityPlayer player && player.fakePlayer != null) {
            this.removeEntity(player.fakePlayer);
        }
    }
    ...
    public boolean invalidateRegionAndSetBlock(BlockPos pos, IBlockState state) {
        ...
        int j2 = pos.getY();
        int k2 = pos.getZ();
        this.invalidateBlockReceiveRegion(i, j2, k2, i, j2, k2);
    ...
    public void doVoidFogParticles(int posX, int posY, int posZ) {
        ...
        for (int j2 = 0; j2 < 1000; ++j2) {
            int k2 = posX + this.rand.nextInt(i) - this.rand.nextInt(i);
            int l2 = posY + this.rand.nextInt(i) - this.rand.nextInt(i);
    ...
    @Override
    public void playSound(double x2, double y2, double z, String soundName, float volume, float pitch, boolean distanceDelay) {
        double d0 = this.mc.getRenderViewEntity().getDistanceSq(x2, y2, z);
        PositionedSoundRecord positionedsoundrecord = new PositionedSoundRecord(new ResourceLocation(soundName), volume, pitch, (float)x2, (float)y2, (float)z);
    ...
    @Override
    public void makeFireworks(double x2, double y2, double z, double motionX, double motionY, double motionZ, NBTTagCompound compund) {
        this.mc.effectRenderer.addEffect(new EntityFirework.StarterFX(this, x2, y2, z, motionX, motionY, motionZ, this.mc.effectRenderer, compund));
    }
