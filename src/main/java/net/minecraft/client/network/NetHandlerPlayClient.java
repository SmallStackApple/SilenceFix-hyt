...
public class NetHandlerPlayClient
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        EventPacketReceive event = new EventPacketReceive(packetIn);
        EventManager.call(event);
        if (event.isCancelled()) {
            return;  // 如果被模块cancel，就不继续处理包了
        }
    ...
    public void handleDisconnect(S40PacketDisconnect packetIn) {
        IChatComponent reason = packetIn.getReason();

        if (reason != null) {
            final String message = reason.getUnformattedText();

            if (message.contains("由于您涉嫌作弊被禁止登录游戏")) {
                if (message.contains("364天") || message.contains("13天") || message.contains("6天")) {
                    reason = new ChatComponentText(
                            EnumChatFormatting.GOLD + "被花雨庭封禁的时间：14/365天  "+EnumChatFormatting.YELLOW+" 学生党，你玩的很好，你是被花雨庭客服封禁的，与你的操作无关！但请注意，封号三次就会IPban。\n"
                                    + EnumChatFormatting.RED + "新号也封就属于IPban  "+EnumChatFormatting.GREEN+" 修复IPban教学：  "+EnumChatFormatting.RED+"拔出光猫电源，等待30秒，然后接入电源即可。光猫不是路由器，是小机顶盒。不懂可百度。"
                    );
                } else if (message.contains("2天")) {
                    reason = new ChatComponentText(
                            EnumChatFormatting.GOLD + "被花雨庭封禁的时间：3天  "+EnumChatFormatting.YELLOW+" 学生党，你被反作弊封禁了，请去群公告观看视频教学，学习如何正确的使用客户端。封号三次就会IPban。\n"
                                    + EnumChatFormatting.RED + "新号也封就属于IPban  "+EnumChatFormatting.GREEN+" 修复IPban教学：  "+EnumChatFormatting.RED+"拔出光猫电源，等待30秒，然后接入电源即可。光猫不是路由器，是小机顶盒。不懂可百度。"
                    );
                }
            }
    ...
    public void handleOpenWindow(S2DPacketOpenWindow packetIn) {
        if (PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController)) return;
