...
extends GuiScreen {
    ...
    public void initGui() {
        ...
        for (GameSettings.Options options : optionsArr) {
            if (options.getEnumFloat()) {
                this.buttonList.add(new GuiOptionSlider(options.returnEnumOrdinal(), x1 + i % 2 * 160, 18 + 24 * (i >> 1), options));
            } else {
                this.buttonList.add(new GuiOptionButton(options.returnEnumOrdinal(), x1 + i % 2 * 160, 18 + 24 * (i >> 1), options, this.options.getKeyBinding(options)));
            }
            ++i;
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GuiControls.drawCenteredString(this.fontRendererObj, this.screenTitle, this.width >> 1, 8, 0xFFFFFF);
        boolean flag = true;
