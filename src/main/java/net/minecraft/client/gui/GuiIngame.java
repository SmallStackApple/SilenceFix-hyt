...
import dev.xinxin.module.modules.render.Health;
...
import static dev.xinxin.module.Module.getModule;
...
extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        PostProcessing postProcessing = SilenceFix.instance.moduleManager.getModule(PostProcessing.class);
        ...
        if (this.showCrosshair()) {
            ...
            this.drawTexturedModalRect(i / 2 - 7, j2 / 2 - 7, 0, 0, 16, 16);
        }
        GlStateManager.enableAlpha();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
        ...
        if (this.mc.thePlayer.getSleepTimer() > 0) {
            ...
            GuiIngame.drawRect(0.0, 0.0, i, j2, k2);
            GlStateManager.enableAlpha();
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                GlStateManager.translate(i / 2, j2 - 68, 0.0f);
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                int l2 = 0xFFFFFF;
                if (this.recordIsPlaying) {
                    l2 = MathHelper.hsvToRGB(f2 / 50.0f, 0.7f, 0.6f) & 0xFFFFFF;
                }
        ...
        if (this.titlesTimer > 0) {
            ...
            if ((i2 = MathHelper.clamp_int(i2, 0, 255)) > 8) {
                GlStateManager.pushMatrix();
                GlStateManager.translate(i / 2, j2 / 2, 0.0f);
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                GlStateManager.pushMatrix();
                GlStateManager.scale(4.0f, 4.0f, 4.0f);
                int j22 = i2 << 24 & 0xFF000000;
                ...
                this.getFontRenderer().drawString(this.displayedSubTitle, -this.getFontRenderer().getStringWidth(this.displayedSubTitle) / 2, SilenceFix.instance.moduleManager.getModule(Health.class).getState() ? 25.0f : 5.0f, 0xFFFFFF | j22, true);
                GlStateManager.popMatrix();
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
...
//        }
        ...
        if (!getModule(HUD.class).state &&this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null)) {
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            this.drawTexturedModalRect(i - 91 - 1 + SilenceFix.instance.getSlotSpoofManager().getSpoofedSlot() * 20, sr.getScaledHeight() - 22 - 1, 0, 22, 24, 22);
            zLevel = f;
    ...
    public void renderHorseJumpBar(ScaledResolution scaledRes, int x2) {
        ...
        int i = 182;
        int j2 = (int)(f * (float)(i + 1));
        int k2 = scaledRes.getScaledHeight() - 32 + 3;
        this.drawTexturedModalRect(x2, k2, 0, 84, i, 5);
        if (j2 > 0) {
    ...
    public void renderSelectedItem(ScaledResolution scaledRes) {
        ...
        if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null) {
            ...
            int i = (scaledRes.getScaledWidth() - this.getFontRenderer().getStringWidth(s2)) / 2;
            ...
            if (k2 > 0) {
                ...
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                this.getFontRenderer().drawStringWithShadow(s2, i, j2, 0xFFFFFF + (k2 << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        }
    ...
    public void renderDemo(ScaledResolution scaledRes) {
        ...
        int i = this.getFontRenderer().getStringWidth(s2);
        this.getFontRenderer().drawStringWithShadow(s2, scaledRes.getScaledWidth() - i - 10, 5.0f, 0xFFFFFF);
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            if (j == 1) {
                this.getFontRenderer().drawString(EnumChatFormatting.AQUA + "免费heshuyou.xyz", l1, k, 553648127);
            ...
            if (j == collection.size()) {
                String s3 = org.apache.commons.lang3.StringUtils.replace(org.apache.commons.lang3.StringUtils.replace(objective.getDisplayName(), "免费获取", "heshuyou.xyz"), "§c✿", "§c⌨");
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int j22 = k1 - (l1 - 1) * i2 - 10;
            float f2 = f1;
            int k22 = entityplayer.getTotalArmorValue();
            int l22 = -1;
            if (entityplayer.isPotionActive(Potion.regeneration)) {
                l22 = this.updateCounter % MathHelper.ceiling_float_int(f + 5.0f);
            }
    ...
    private void renderBossHealth() {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            this.getFontRenderer().drawStringWithShadow(s2, i / 2 - this.getFontRenderer().getStringWidth(s2) / 2, i1 - 10, 0xFFFFFF);
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }
    ...
    public void displayTitle(String title, String subTitle, int timeFadeIn, int displayTime, int timeFadeOut) {
        if (title == null && subTitle == null && timeFadeIn < 0 && displayTime < 0 && timeFadeOut < 0) {
            ...
            this.displayedTitle = title
                    .replace("花雨庭", "欢迎您购买欣欣内部！爱来自李先于")
                    .replace("VICTORY", "" + LiveClient.INSTANCE.liveUser.getName() + "获得了本场胜利！")
                    .replace("战斗开始", "杀戮和暴击是绑定一起的！快捷键是R哦~")
                    .replace("匹配成功", "请一定要看好教程！"); // 最大的字

            if (subTitle != null) {
                this.displayedSubTitle = subTitle
                        .replace("新活动已上线..", "爱来自李先于")
                        .replace("恭喜获得胜利~!", "~记得看好使用教程哦~")
                        .replace("准备开始...", "请多害死傻逼欣欣IRC用户")
                        .replace("开始传送...", "否则会嘎嘎封号！")
                        .replace("Fighting", "这样才能发挥百分百的实力！")
                        .replace("准备战斗...", "自动金苹果最好6个心就开启哦！");
                System.out.println(displayedSubTitle);
            }
