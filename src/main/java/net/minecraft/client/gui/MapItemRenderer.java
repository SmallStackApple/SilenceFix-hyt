    ...
    class Instance {
        ...
        private void updateMapTexture() {
            for (int i = 0; i < 16384; ++i) {
                int j2 = this.mapData.colors[i] & 0xFF;
                this.mapTextureData[i] = j2 / 4 == 0 ? (i + i / 128 & 1) * 8 + 16 << 24 : MapColor.mapColorArray[j2 / 4].getMapColor(j2 & 3);
            }
        ...
        private void render(boolean noOverlayRendering) {
            ...
            worldrenderer.pos((float)(i + 0) + f, (float)(j2 + 128) - f, -0.01f).tex(0.0, 1.0).endVertex();
            worldrenderer.pos((float)(i + 128) - f, (float)(j2 + 128) - f, -0.01f).tex(1.0, 1.0).endVertex();
            worldrenderer.pos((float)(i + 128) - f, (float)(j2 + 0) + f, -0.01f).tex(1.0, 0.0).endVertex();
            worldrenderer.pos((float)(i + 0) + f, (float)(j2 + 0) + f, -0.01f).tex(0.0, 0.0).endVertex();
            tessellator.draw();
            ...
            for (Vec4b vec4b : this.mapData.mapDecorations.values()) {
                ...
                GlStateManager.translate((float)i + (float)vec4b.func_176112_b() / 2.0f + 64.0f, (float)j2 + (float)vec4b.func_176113_c() / 2.0f + 64.0f, -0.02f);
                ...
                worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
                float f5 = -0.001f;
                worldrenderer.pos(-1.0, 1.0, (float)k2 * -0.001f).tex(f1, f2).endVertex();
                worldrenderer.pos(1.0, 1.0, (float)k2 * -0.001f).tex(f3, f2).endVertex();
