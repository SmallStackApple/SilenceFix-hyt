...
extends GuiContainer {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (int k2 = 0; k2 < 3; ++k2) {
            ...
            if (l2 < 0 || i1 < 0 || l2 >= 108 || i1 >= 19 || !this.container.enchantItem(this.mc.thePlayer, k2)) continue;
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        for (int l2 = 0; l2 < 3; ++l2) {
            ...
            int j1 = i1 + 20;
            int k1 = 86;
            String s2 = EnchantmentNameParts.getInstance().generateNewRandomName();
            zLevel = 0.0f;
            this.mc.getTextureManager().bindTexture(ENCHANTMENT_TABLE_GUI_TEXTURE);
            int l1 = this.container.enchantLevels[l2];
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            if (l1 == 0) {
            ...
            String s1 = "" + l1;
            FontRenderer fontrenderer = this.mc.standardGalactiRapeMasterFontManager;
            int i2 = 6839882;
            if (!(k2 >= l2 + 1 && this.mc.thePlayer.experienceLevel >= l1 || this.mc.thePlayer.capabilities.isCreativeMode)) {
                this.drawTexturedModalRect(i1, j2 + 14 + 19 * l2, 0, 185, 108, 19);
                this.drawTexturedModalRect(i1 + 1, j2 + 15 + 19 * l2, 16 * l2, 239, 16, 16);
                fontrenderer.drawSplitString(s2, j1, j2 + 16 + 19 * l2, k1, (i2 & 0xFEFEFE) >> 1);
                i2 = 4226832;
            } else {
                int j22 = mouseX - (i + 60);
                ...
                fontrenderer.drawSplitString(s2, j1, j2 + 16 + 19 * l2, k1, i2);
                i2 = 8453920;
            }
            fontrenderer = this.mc.fontRendererObj;
            fontrenderer.drawStringWithShadow(s1, j1 + 86 - fontrenderer.getStringWidth(s1), j2 + 16 + 19 * l2 + 7, i2);
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int j2 = 0; j2 < 3; ++j2) {
            ...
            int l2 = this.container.enchantmentIds[j2];
            int i1 = j2 + 1;
            if (!this.isPointInRegion(60, 14 + 19 * j2, 108, 17, mouseX, mouseY) || k2 <= 0 || l2 < 0) continue;
            ArrayList list = Lists.newArrayList();
            if (l2 >= 0 && Enchantment.getEnchantmentById(l2 & 0xFF) != null) {
                String s2 = Enchantment.getEnchantmentById(l2 & 0xFF).getTranslatedName((l2 & 0xFF00) >> 8);
                list.add(EnumChatFormatting.WHITE.toString() + EnumChatFormatting.ITALIC.toString() + I18n.format("container.enchant.clue", s2));
            }
            if (!flag) {
                if (l2 >= 0) {
                    list.add("");
                }
                if (this.mc.thePlayer.experienceLevel < k2) {
                    list.add(EnumChatFormatting.RED.toString() + "Level Requirement: " + this.container.enchantLevels[j2]);
                } else {
                    String s1 = "";
