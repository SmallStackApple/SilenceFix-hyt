...
implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x2, int y2, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
        this.mc.fontRendererObj.drawString(I18n.format("lanServer.title", new Object[0]), x2 + 32 + 3, y2 + 1, 0xFFFFFF);
        this.mc.fontRendererObj.drawString(this.field_148291_b.getServerMotd(), x2 + 32 + 3, y2 + 12, 0x808080);
        if (this.mc.gameSettings.hideServerAddress) {
            this.mc.fontRendererObj.drawString(I18n.format("selectServer.hiddenAddress", new Object[0]), x2 + 32 + 3, y2 + 12 + 11, 0x303030);
        } else {
            this.mc.fontRendererObj.drawString(this.field_148291_b.getServerIpPort(), x2 + 32 + 3, y2 + 12 + 11, 0x303030);
        }
    }
