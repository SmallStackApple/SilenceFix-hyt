...
import com.xinxin.client.viaversion.viamcp.gui.GuiProtocolSelector;
...
implements GuiYesNoCallback {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        switch (button.id) {
            ...
            case 2: {
                if (guilistextended$iguilistentry instanceof ServerListEntryLanDetected){
                    var serverListEntryLanDetected = (ServerListEntryLanDetected)guilistextended$iguilistentry;
                    if (serverListEntryLanDetected.field_148291_b instanceof GuiAltManager.ProxyServer){
                        GuiAltManager.proxyServers.remove(serverListEntryLanDetected.field_148291_b);
                        this.refreshServerList();
                        return;
                    }
                }
                String s4;
                if (!(guilistextended$iguilistentry instanceof ServerListEntryNormal) || (s4 = ((ServerListEntryNormal)guilistextended$iguilistentry).getServerData().serverName) == null) break;
            ...
            case 7: {
                if (!(guilistextended$iguilistentry instanceof ServerListEntryNormal)) break;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GuiMultiplayer.drawCenteredString(this.fontRendererObj, I18n.format("multiplayer.title", new Object[0]), this.width / 2, 20, 0xFFFFFF);
    ...
    public void connectToSelected() {
        GuiAltManager.currentProxyServer = null;
        GuiListExtended.IGuiListEntry guilistextended$iguilistentry;
        GuiListExtended.IGuiListEntry iGuiListEntry = guilistextended$iguilistentry = this.serverListSelector.func_148193_k() < 0 ? null : this.serverListSelector.getListEntry(this.serverListSelector.func_148193_k());
        if (guilistextended$iguilistentry instanceof ServerListEntryNormal) {
            ...
            var serverListEntryLanDetected = (ServerListEntryLanDetected)guilistextended$iguilistentry;
            if (serverListEntryLanDetected.field_148291_b instanceof GuiAltManager.ProxyServer){
                GuiAltManager.currentProxyServer = (GuiAltManager.ProxyServer) serverListEntryLanDetected.field_148291_b;
                mc.session = new Session(GuiAltManager.currentProxyServer.roleName, UUID.randomUUID().toString(),"","");
            }
    ...
    public void selectServer(int index) {
        ...
        if (guilistextended$iguilistentry != null) {
            if (!(guilistextended$iguilistentry instanceof ServerListEntryLanScan)){
                ...
                if (guilistextended$iguilistentry instanceof ServerListEntryLanDetected){
                    var serverListEntryLanScan = (ServerListEntryLanDetected)guilistextended$iguilistentry;
                    if (serverListEntryLanScan.field_148291_b instanceof GuiAltManager.ProxyServer){
                        this.btnDeleteServer.enabled = true;
                    }
                }
            }else{
