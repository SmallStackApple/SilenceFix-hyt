...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn)
    {
        ...
        EntityPlayer tabPlayer = mc.theWorld.getPlayerEntityByName(networkPlayerInfoIn.getGameProfile().getName());
        if (tabPlayer != null) {
            final String name = networkPlayerInfoIn.getGameProfile().getName();

            Optional.ofNullable(LiveClient.INSTANCE.getLiveComponent().getHackerMap().get(name))
                    .map(HackerPlayer::getClient)
                    .ifPresent((usingClient) -> {
                        switch (usingClient) {
                            case SOUTHSIDE -> sb.append(EnumChatFormatting.GOLD)
                                    .append(EnumChatFormatting.BOLD)
                                    .append("[SouthSide] ")
                                    .append(EnumChatFormatting.RESET);
                            case GUARD_FIX -> sb.append(EnumChatFormatting.LIGHT_PURPLE)
                                    .append(EnumChatFormatting.ITALIC)
                                    .append("[最强公益客户端] ")
                                    .append(EnumChatFormatting.RESET);
                            case OTHERS -> sb.append(EnumChatFormatting.DARK_RED)
                                    .append(EnumChatFormatting.BOLD)
                                    .append("[Hacker] ")
                                    .append(EnumChatFormatting.RESET);
                        }
        ...
        if (networkPlayerInfoIn.liveUser != null) {
            sb.append(LiveComponent.getLiveUserDisplayName(networkPlayerInfoIn.liveUser));
        }

        if (networkPlayerInfoIn.getDisplayName() != null) {
            sb.append(networkPlayerInfoIn.getDisplayName().getFormattedText());
        } else {
            sb.append(ScorePlayerTeam.formatPlayerName(networkPlayerInfoIn.getPlayerTeam(), networkPlayerInfoIn.getGameProfile().getName()));
        }

        return sb.toString().replaceAll("Veta","§c§lVeta").replaceAll("www.hyt.com","heshuyou.xyz").replaceAll("§b§l花雨庭","§c§lSilenceFix");
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
        ...
        if (this.header != null) {
            list1 = this.mc.fontRendererObj.listFormattedStringToWidth(this.header.getFormattedText().replace("花雨庭", "欣欣公益").replace("❀", "⌨").replace("heshuyou.xyz", "heshuyou.xyz"), width - 50);
        ...
        if (this.header != null) {
            list1 = this.mc.fontRendererObj.listFormattedStringToWidth(this.header.getFormattedText().replace("Veta", "欣欣公益").replace("❀", "⌨").replace("heshuyou.xyz", "heshuyou.xyz"), width - 50);
        ...
        if (this.footer != null) {
            list2 = this.mc.fontRendererObj.listFormattedStringToWidth(this.footer.getFormattedText().replace("heshuyou.xyz", "heshuyou.xyz"), width - 50);
        ...
        if (this.footer != null) {
            list2 = this.mc.fontRendererObj.listFormattedStringToWidth(this.footer.getFormattedText().replace("Powered by Veta Team", "官方QQ群：205521532 欣欣制作 内置进服！").replace("www.mchyt.com","205521532 欣欣制作").replace("官网","官方QQ群"), width - 50);
        ...
        if (list1 != null) {
            ...
            for (String s3 : list1) {
                ...
                this.mc.fontRendererObj.drawStringWithShadow(s3.replaceAll("花雨庭","§b§l欣欣公益"), (float) (width / 2 - i2 / 2), (float) k1, -1);
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                this.drawPing(i1, j2 - (flag ? 11 : 0), k2, networkplayerinfo1);
            }
        }
