...
implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x2, int y2, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
        ...
        for (int i = 0; i < Math.min(list.size(), 2); ++i) {
            this.mc.fontRendererObj.drawString(list.get(i), x2 + 32 + 3, y2 + 12 + this.mc.fontRendererObj.FONT_HEIGHT * i, 0x808080);
        }
        ...
        if (flag2) {
            l2 = 5;
            s1 = flag ? "SilenceFix out of date!" : "Server out of date!";
        ...
        Gui.drawModalRectWithCustomSizedTexture(x2 + listWidth - 15, y2, (float)(k2 * 10), (float)(176 + l2 * 8), 10, 8, 256.0f, 256.0f);
