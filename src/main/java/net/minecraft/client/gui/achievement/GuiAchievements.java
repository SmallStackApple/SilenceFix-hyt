...
implements IProgressMeter {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (this.loadingAchievements) {
            ...
            GuiAchievements.drawCenteredString(this.fontRendererObj, I18n.format("multiplayer.downloadingStats", new Object[0]), this.width / 2, this.height / 2, 0xFFFFFF);
            GuiAchievements.drawCenteredString(this.fontRendererObj, lanSearchStates[(int)(Minecraft.getSystemTime() / 150L % (long)lanSearchStates.length)], this.width / 2, this.height / 2 + this.fontRendererObj.FONT_HEIGHT * 2, 0xFFFFFF);
        } else {
    ...
    protected void drawTitle() {
        int i = (this.width - this.field_146555_f) / 2;
        int j2 = (this.height - this.field_146557_g) / 2;
        this.fontRendererObj.drawString(I18n.format("gui.achievements", new Object[0]), i + 15, j2 + 5, 0x404040);
    }
    ...
    protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_) {
        ...
        int l3 = 0;
        while ((float)l3 * f - (float)j22 < 155.0f) {
            ...
            while ((float)i4 * f1 - (float)i2 < 224.0f) {
                ...
                this.drawTexturedModalRect(i4 * 16 - i2, l3 * 16 - j22, textureatlassprite, 16, 16);
                ++i4;
            }
            ++l3;
        }
