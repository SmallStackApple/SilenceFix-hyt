...
extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ItemStack itemstack;
        ChestStealer stealer = SilenceFix.instance.moduleManager.getModule(ChestStealer.class);
        if (stealer.getState() && stealer.silentValue.getValue()) {
            ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft());
            Minecraft mc = Minecraft.getMinecraft();
            GuiScreen guiScreen = mc.currentScreen;
            long time = System.currentTimeMillis();
            float pulse = (float)Math.sin(time / 200.0) * 0.1f + 1.0f;
            int circleColor = 0xFF4ECCF5; // 浅蓝色
            ...
            if (guiScreen instanceof GuiChest) {
                ...
                if (!stealer.silentValue.getValue() || chest.lowerChestInventory != null) {
                    ...
                    if (stealer.silentValue.getValue()) {
                        if (true){
                            return;
                        }
                        GlStateManager.pushMatrix();
                        GlStateManager.translate(sr.getScaledWidth() / 2, sr.getScaledHeight() / 2.0f + 25.0f, 0);
                        GlStateManager.rotate((time % 1000) / 1000.0f * 360.0f, 0, 0, 1);
                        GlStateManager.color(0.31f, 0.8f, 0.96f, 1.0f); // RGB对应circleColor
                        RenderUtil.drawLoadingCircle(0, 0);
                        GlStateManager.popMatrix();
                        String tipString = "Stealing";
                        GlStateManager.pushMatrix();
                        GlStateManager.translate(
                                (float)this.width / 2.0f,
                                (float)this.height / 2.0f + 40.0f,
                                0
                        );
                        GlStateManager.scale(pulse, pulse, 1.0f);
                        FontManager.arial18.drawString(
                                tipString,
                                -(float)FontManager.arial18.getStringWidth(tipString) / 2.0f,
                                0,
                                textColor,
                                false
                        );
            ...
            if (guiScreen instanceof GuiFurnace || guiScreen instanceof GuiBrewingStand) {
                ...
                if (stealer.silentValue.getValue()) {
                    ...
                    GlStateManager.translate(sr.getScaledWidth() / 2, sr.getScaledHeight() / 2.0f + 25.0f, 0);
                    GlStateManager.rotate((time % 1000) / 1000.0f * 360.0f, 0, 0, 1);
                    GlStateManager.color(0.31f, 0.8f, 0.96f, 1.0f);
                    RenderUtil.drawLoadingCircle(0, 0);
                    GlStateManager.popMatrix();
                    String tipString = "Stealing";
                    GlStateManager.pushMatrix();
        ...
        GlStateManager.pushMatrix();
        GlStateManager.translate(i, j2, 0.0f);
        ...
        if (itemstack != null) {
            ...
            this.drawItemStack(itemstack, mouseX - i - j22, mouseY - j2 - k22, s2);
        }
        if (this.returningStack != null) {
            ...
            int l1 = this.touchUpX + (int)((float)l22 * f);
    ...
    private void drawSlot(Slot slotIn) {
        ...
        if (itemstack == null && (s1 = slotIn.getSlotTexture()) != null) {
            ...
            this.drawTexturedModalRect(i, j2, textureatlassprite, 16, 16);
            GlStateManager.enableLighting();
            flag1 = true;
        }
        if (!flag1) {
            if (flag) {
                GuiContainer.drawRect(i, j2, i + 16, j2 + 16, -2130706433);
            }
            GlStateManager.enableDepth();
            this.itemRender.renderItemAndEffectIntoGUI(itemstack, i, j2);
            this.itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, itemstack, i, j2, s2);
        }
        this.itemRender.zLevel = 0.0f;
        zLevel = 0.0f;
    }
    ...
    public Slot getSlotAtPosition(int x2, int y2) {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        int i = this.guiLeft;
        int j2 = this.guiTop;
        boolean flag = mouseX < i || mouseY < j2 || mouseX >= i + this.xSize || mouseY >= j2 + this.ySize;
        int k2 = -1;
        if (slot != null) {
            k2 = slot.slotNumber;
        }
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(null, slot)) {
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
                ...
                if (state == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                    ...
                    boolean flag1;
                    boolean bl = flag1 = k2 != -999 && (Keyboard.isKeyDown((int)42) || Keyboard.isKeyDown((int)54));
                    if (flag1) {
                        this.shiftClickedSlot = slot != null && slot.getHasStack() ? slot.getStack() : null;
                    }
    ...
    protected boolean isPointInRegion(int left, int top, int right, int bottom, int pointX, int pointY) {
        int i = this.guiLeft;
        int j2 = this.guiTop;
        return (pointX -= i) >= left - 1 && pointX < left + right + 1 && (pointY -= j2) >= top - 1 && pointY < top + bottom + 1;
    }
