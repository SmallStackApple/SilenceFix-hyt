...
extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (this.horseEntity.isChested()) {
            this.drawTexturedModalRect(i + 79, j2 + 17, 0, this.ySize, 90, 54);
        }
        if (this.horseEntity.canWearArmor()) {
            this.drawTexturedModalRect(i + 7, j2 + 35, 0, this.ySize + 54, 18, 18);
        }
        GuiInventory.drawEntityOnScreen(i + 51, j2 + 60, 17, (float)(i + 51) - this.mousePosx, (float)(j2 + 75 - 50) - this.mousePosY, this.horseEntity);
    }
