...
extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        for (int j2 = 4; j2 < 8; ++j2) {
            ...
            this.blazeSticks[j2].rotationPointX = MathHelper.cos(f) * 7.0f;
            this.blazeSticks[j2].rotationPointZ = MathHelper.sin(f) * 7.0f;
            f += 1.0f;
        }
        f = 0.47123894f + ageInTicks * (float)Math.PI * -0.05f;
        for (int k2 = 8; k2 < 12; ++k2) {
            this.blazeSticks[k2].rotationPointY = 11.0f + MathHelper.cos(((float)k2 * 1.5f + ageInTicks) * 0.5f);
            this.blazeSticks[k2].rotationPointX = MathHelper.cos(f) * 5.0f;
            this.blazeSticks[k2].rotationPointZ = MathHelper.sin(f) * 5.0f;
            f += 1.0f;
        }
