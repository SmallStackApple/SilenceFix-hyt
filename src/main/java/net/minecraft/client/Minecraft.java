...
import dev.xinxin.module.modules.misc.BalancedTimer;
...
import dev.xinxin.utils.PacketProcess.PacketProcessCallable;
import dev.xinxin.utils.PacketProcess.PacketProcessListenableFutureTask;
import dev.xinxin.utils.PacketProcess.PacketProcessRunnable;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("ysxiaoyucrk" + " SilenceFixPro"+SilenceFix.VERSION+" Welcome User 爱来自李先于 ");
    }
    @JNICExclude
    ...
    public void displayCrashReport(CrashReport crashReportIn) {
        ...
        if (XRay.isEnabled) {
            SilenceFix.instance.moduleManager.getModule(XRay.class).toggle();
        }
    ...
    public void shutdownMinecraftApplet() {
        try {
            ...
            for (Module m : SilenceFix.instance.moduleManager.getModules()) {
            ...
            try {
                final String crashMessage = LiveClient.INSTANCE.getCrashMessage().get();

                if (crashMessage != null) {
                    JOptionPane.showMessageDialog(null, crashMessage);
                }
            } catch (Throwable e) {
                logger.error(e);
            }
...
//                this.runTick();
//                if (this.lastTickSentC03 || lastTickSkipped) continue;
//                Queue<FutureTask<?>> queue = this.scheduledTasks;
//                synchronized (queue) {
//                    while (!this.scheduledTasks.isEmpty()) {
//                        Util.runTask(this.scheduledTasks.poll(), logger);
...
//    }
    ...
    private void runGameLoop() throws IOException {
        ...
        AnimationUtil.deltaTime = deltaTime;
    ...
    private void displayDebugInfo() {
        if (this.mcProfiler.profilingEnabled) {
            ...
            worldrenderer.pos((float)j2 - (float)i * 1.1f, (float)k2 - (float)i * 0.6f - 16.50f, 0.0).color(200, 0, 0, 0).endVertex();
    ...
    public void sendClickBlockToController(boolean leftClick) {
        ...
        if (this.leftClickCounter <= 0 && !this.thePlayer.isUsingItem()) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (LiveComponent.isBlockBreakable(blockpos, blockState, block) == null) {
                    if (block.getMaterial() != Material.air && this.playerController.onPlayerDamageBlock(blockpos, this.objectMouseOver.sideHit)) {
                ...
                switch (this.objectMouseOver.typeOfHit) {
                    ...
                    case BLOCK: {
                        ...
                        if (block.getMaterial() != Material.air) {
                            final NetworkPlayerInfo owner = LiveComponent.isBlockBreakable(blockpos, blockState, block);

                            if (owner == null) {
                                this.playerController.clickBlock(blockpos, this.objectMouseOver.sideHit);
                            } else {
                                HelperUtil.sendMessage("§f你无法破坏这个床！！！因为这个队伍有§e何树友：" + (owner.liveUser == null ? Objects.toString(owner.getDisplayName()) : owner.liveUser.getName()));
                            }
                            break;
                        }
                    }
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            for (int l = 0; l < 9; ++l) {
                if (this.gameSettings.keyBindsHotbar[l].isPressed()) {
                    ...
                    if (BalancedTimer.isInstanceEnabled() && Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && (Keyboard.isKeyDown(Keyboard.KEY_1) || Keyboard.isKeyDown(Keyboard.KEY_2))) {
                        return;
                    }
        ...
        try {
            {
                if (getMinecraft().thePlayer != null && getMinecraft().theWorld != null && getMinecraft().thePlayer.ticksExisted > 10 && Shaders.configAntialiasingLevel == 0) {
                    ...
                    MotionBlur motionBlur = SilenceFix.instance.moduleManager.getModule(MotionBlur.class);
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        this.systemTime = 0L;
        if (!isSingleplayer() && thePlayer != null) {
            LiveClient.INSTANCE.sendPacket(LiveProto.createUpdateMinecraftProfile(thePlayer.getUniqueID(), thePlayer.getName()));
        }
    }
    @JNICExclude
    ...
    public CrashReport addGraphicsAndWorldToCrashReport(CrashReport theCrash) {
        ...
        theCrash.getCategory().addCrashSectionCallable("Is Modded", () -> {
            ...
            return !s2.equals("vanilla") ? "Definitely; SilenceFix brand changed to '" + s2 + "'" : (Minecraft.class.getSigners() == null ? "Very likely; Jar signature invalidated" : "Probably not. Jar signature remains and client brand is untouched.");
        });
        theCrash.getCategory().addCrashSectionCallable("Type", () -> "SilenceFix (map_client.txt)");
