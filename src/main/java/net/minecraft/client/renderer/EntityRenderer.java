...
import dev.xinxin.module.modules.render.Camera;
import dev.xinxin.module.modules.render.NoHurtCam;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            boolean zoomKeyDown = GameSettings.isKeyDown(this.mc.gameSettings.ofKeyBindZoom);
            if (zoomKeyDown && !(this.mc.currentScreen instanceof GuiChat)) {
                f /= 4.0F; // 视野缩小 4 倍（和 1.12.2 完全一致）
            }
            if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).getHealth() <= 0.0F) {
                float deathTime = (float)((EntityLivingBase)entity).deathTime + partialTicks;
                f /= (1.0F - 500.0F / (deathTime + 500.0F)) * 2.0F + 1.0F;
            }
            if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isInsideOfMaterial(Material.water)) {
                f *= 60.0F / 70.0F;
            }
            Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
            if (Reflector.ForgeHooksClient_getFOVModifier.exists()) {
                f = Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier,
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase)
        {
            ...
            if(SilenceFix.instance.moduleManager.getModule(NoHurtCam.class).getState()) {
                return;
            }
    ...
    {
        Camera camera = SilenceFix.instance.moduleManager.getModule(Camera.class);
        Entity entity = this.mc.getRenderViewEntity();
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                if (camera.motionCamera.getValue()) {
    ...
    private void updateMainMenu(BetterMainMenu p_updateMainMenu_1_)
    {
        try
        {
            ...
            Reflector.setFieldValue(p_updateMainMenu_1_, Reflector.BetterMainMenu_splashText, s);
        }
