...
import dev.xinxin.module.modules.render.BlockHit;
...
public class ItemRenderer
{
    ...
    private TimerUtil rT = new TimerUtil();
    ...
    private void rotateItemAnim() {
        if (BlockHit.transformFirstPersonRotate.getValue().equals(BlockHit.RotateMode.RotateY)) {
            GlStateManager.rotate(this.delay, 0.0F, 1.0F, 0.0F);
        }
        if (BlockHit.transformFirstPersonRotate.getValue().equals(BlockHit.RotateMode.RotateXY)) {
            GlStateManager.rotate(this.delay, 1.0F, 1.0F, 0.0F);
        }

        if (BlockHit.transformFirstPersonRotate.getValue().equals(BlockHit.RotateMode.Custom)) {
            GlStateManager.rotate(this.delay, BlockHit.customRotate1.getValue().floatValue(), BlockHit.customRotate2.getValue().floatValue(), BlockHit.customRotate3.getValue().floatValue());
        }

        if (this.rT.hasTimePassed(1)) {
            ++this.delay;
                this.delay = this.delay + BlockHit.SpeedRotate.getValue().floatValue();
            this.rT.reset();
        }
    ...
    {
        boolean animations = SilenceFix.instance.moduleManager.getModule(BlockHit.class).getState();
        double x = animations ? .56 + (BlockHit.x.getValue() * .01) : .56;
        double y = animations ? .52 - (BlockHit.y.getValue() * .01) : .52;
        double size = animations ? .40 + (BlockHit.size.getValue() * .01) : .40;
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            BlockHit block = (BlockHit) SilenceFix.instance.moduleManager.getModule(BlockHit.class);
            ...
            try {
                if (this.itemToRender != null) {
                    ...
                    } else if (abstractclientplayer.getItemInUseCount() > 0 ||  (itemToRender.getItem() instanceof ItemSword && (KillAura.isBlocking))) {
                        float var15 = MathHelper.sin(var4 * var4 * 3.1415927F);
                        float var16 = MathHelper.sin(MathHelper.sqrt_float(var4) * 3.1415927F);
                        ...
                        switch ( enumaction) {
                            ...
                            case BLOCK:
                                ...
                                if (!block.getState()) {
                                    ...
                                    switch (block.blockAnimation.getValue().name()) {
                                        case "Slide": {
                                        ...
                                        case "Middle": {
                                            ...
                                            this.transformFirstPersonItem(0.0f, var4);
                                        ...
                                        case "Smooth":{
                                            this.transformFirstPersonItem(var4 / 5, var4);
                                        ...
                                        case "Swong": {
                                            transformFirstPersonItem(f / 2.0F, var4);
                                        ...
                                        case "Stab": {
                                            final float spin = MathHelper.sin(MathHelper.sqrt_float(var4) * (float) Math.PI);
    ...
    public void updateEquippedItem()
    {
        ...
        ItemStack itemstack = SilenceFix.instance.getSlotSpoofManager().getSpoofedStack();
        ...
        if (this.itemToRender != null && itemstack != null) {
            if (!this.itemToRender.getIsItemStackEqual(itemstack)) {
                if (Reflector.ForgeItem_shouldCauseReequipAnimation.exists()) {
                    boolean flag1 = Reflector.callBoolean(this.itemToRender.getItem(), Reflector.ForgeItem_shouldCauseReequipAnimation, new Object[]{this.itemToRender, itemstack, Boolean.valueOf(this.equippedItemSlot != SilenceFix.instance.getSlotSpoofManager().getSpoofedSlot())});

                    if (!flag1) {
                        this.itemToRender = itemstack;
                        this.equippedItemSlot = SilenceFix.instance.getSlotSpoofManager().getSpoofedSlot();
                        return;
                    }
                }
