...
import dev.xinxin.utils.FakePlayer;
import dev.xinxin.utils.player.RotationUtil;
import dev.xinxin.utils.waveycapes.sim.StickSimulation;
...
public abstract class EntityPlayer
    ...
    public boolean hasQueryIRCUser = false;
    public boolean isHacker = false;
    public boolean isSilenceUser = false;
    ...
    public EntityPlayer(World worldIn, GameProfile gameProfileIn) {
        ...
        if (LiveClient.INSTANCE.skippedLogin && Minecraft.getMinecraft().getNetHandler() != null) {
            for (NetworkPlayerInfo playerInfo : Minecraft.getMinecraft().getNetHandler().getPlayerInfoMap()) {
                if (playerInfo.liveUser != null && (playerInfo.getGameProfile().getId().equals(gameProfileIn.getId()) || playerInfo.getGameProfile().getName().equals(gameProfileIn.getName()))) {
                    liveUser = playerInfo.liveUser;
                }
            }
        }
    }
    ...
    public void simulate(EntityPlayer abstractClientPlayer) {
        ...
        double d2 = abstractClientPlayer.chasingPosX - abstractClientPlayer.posX;
        ...
        double o2 = MathHelper.sin(n * ((float)Math.PI / 180));
        ...
        this.stickSimulation.points.get((int)0).position.x = (float)((double)this.stickSimulation.points.get((int)0).position.x + (d2 * o2 + m * p + fallHack));
            ...
            if (f > 0.0f || f1 > 0.0f) {
                ...
                if (flag2) {
                    ...
                    if (i > 0) {
                        ...
                        EventSlowDown event = new EventSlowDown(EventSlowDown.Type.Sprinting, 0.2f, 0.2f);
    ...
    public void setDead() {
        ...
        EventManager.call(new EventPlayerDead(this));
    }
    ...
    public float getClosestDistanceToEntity(Entity target) {
    ...
    public static UUID getOfflineUUID(String username) {
        return UUID.nameUUIDFromBytes(("何树友:" + username).getBytes(Charsets.UTF_8));
    }
