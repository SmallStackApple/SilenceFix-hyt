...
import dev.xinxin.module.modules.combat.AutoClicker;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4f) {
            ...
            if (SilenceFix.instance.moduleManager.getModule(AutoClicker.class).state && (Double)AutoClicker.jitter.getValue() != 0.0 && Mouse.isButtonDown(0)) {
    ...
    public double getDistance(Entity entity) {
        double x = this.posX - (double)entity.getPosition().getX();
        double y = this.posY - (double)entity.getPosition().getY();
        double z = this.posZ - (double)entity.getPosition().getZ();
    ...
    public MovingObjectPosition rayTraceCustom(double blockReachDistance, float yaw, float pitch, float partialTicks) {
        ...
        final Vec3 vec32 = vec3.add(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, false, true);
    }

    public MovingObjectPosition customRayTrace(double blockReachDistance, float partialTicks, float yaw, float pitch) {
        Vec3 vec3 = this.getPositionEyes(partialTicks);
        Vec3 vec4 = this.getCustomLook(partialTicks, yaw, pitch);
