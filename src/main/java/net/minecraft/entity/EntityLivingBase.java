...
import com.xinxin.client.utils.misc.fixer.MovementFixer;
...
import dev.xinxin.event.world.EventMotionSP;
import dev.xinxin.event.world.MoveMathEvent;
import dev.xinxin.module.ModuleManager;
import dev.xinxin.module.modules.movement.FastLadder;
import dev.xinxin.module.modules.movement.NoJumpDelay;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
        FastLadder fastLadder = (FastLadder) SilenceFix.instance.moduleManager.getModule(FastLadder.class);
        if (fastLadder.getState() && fastLadder.cancel) {
            return false;
        }
    ...
    private int getArmSwingAnimationEnd()
    {
        ...
        EventManager.call(swingAnimationEvent);
    ...
    public void moveEntityWithHeading(float strafe, float forward)
    {
        if (this == Minecraft.getMinecraft().thePlayer) {
            ...
            EventManager.call(moveMathEvent);
            if (moveMathEvent.isCancelled()) return;
        }


        EventMotionSP eventMotionSP = new EventMotionSP();
        EventManager.call(eventMotionSP);

        if (eventMotionSP.isCancelled()) {
            return;
        }

//        if (SilenceFix.instance.moduleManager.getModule(Stuck.class).state && this == Minecraft.getMinecraft().thePlayer) {
//            if (Minecraft.getMinecraft().thePlayer.positionUpdateTicks < 20 && !Stuck.isS12) {
//                return;
//            } else if (Stuck.isS12) Stuck.isS12 = false;
//        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (Math.abs(this.motionX) < MovementFixer.fixMinimumMotion())
        {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < MovementFixer.fixMinimumMotion())
        {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < MovementFixer.fixMinimumMotion())
        {
            this.motionZ = 0.0D;
        }
        ...
        if (this.isJumping)
        {
            ...
            {
                this.jump();
                if (SilenceFix.instance.moduleManager.getModule(NoJumpDelay.class) == null || !SilenceFix.instance.moduleManager.getModule(NoJumpDelay.class).getState()) {
                    this.jumpTicks = 10;
                }
            }
        }
