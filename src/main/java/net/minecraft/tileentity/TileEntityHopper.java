...
ITickable {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, ItemStack stack, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory && side != null) {
            ...
            int i = inventoryIn.getSizeInventory();
            for (int j2 = 0; j2 < i && stack != null && stack.stackSize > 0; ++j2) {
    ...
    private static ItemStack insertStack(IInventory inventoryIn, ItemStack stack, int index, EnumFacing side) {
        ...
        if (TileEntityHopper.canInsertItemInSlot(inventoryIn, stack, index, side)) {
            ...
            if (itemstack == null) {
                ...
                int j2 = Math.min(stack.stackSize, i);
                stack.stackSize -= j2;
    ...
    public static IInventory getInventoryAtPosition(World worldIn, double x2, double y2, double z) {
        List<Entity> list;
        TileEntity tileentity;
        int k2;
        int j2;
        IInventory iinventory = null;
        int i = MathHelper.floor_double(x2);
        BlockPos blockpos = new BlockPos(i, j2 = MathHelper.floor_double(y2), k2 = MathHelper.floor_double(z));
        ...
        if (iinventory == null && (list = worldIn.getEntitiesInAABBexcluding(null, new AxisAlignedBB(x2 - 0.5, y2 - 0.5, z - 0.5, x2 + 0.5, y2 + 0.5, z + 0.5), EntitySelectors.selectInventories)).size() > 0) {
