...
public abstract class MobSpawnerBaseLogic {
        ...
        if (this.isActivated()) {
            ...
            if (this.getSpawnerWorld().isRemote) {
                ...
                for (int i = 0; i < this.spawnCount; ++i) {
                    ...
                    int j2 = this.getSpawnerWorld().getEntitiesWithinAABB(entity.getClass(), new AxisAlignedBB(blockpos.getX(), blockpos.getY(), blockpos.getZ(), blockpos.getX() + 1, blockpos.getY() + 1, blockpos.getZ() + 1).expand(this.spawnRange, this.spawnRange, this.spawnRange)).size();
                    if (j2 >= this.maxNearbyEntities) {
                        this.resetTimer();
                        return;
                    }
    ...
    private Entity spawnNewEntity(Entity entityIn, boolean spawn) {
        if (this.getRandomEntity() != null) {
            ...
            for (String s2 : this.getRandomEntity().nbtData.getKeySet()) {
                NBTBase nbtbase = this.getRandomEntity().nbtData.getTag(s2);
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        String s2 = this.getEntityNameToSpawn();
        if (!StringUtils.isNullOrEmpty(s2)) {
            nbt.setString("EntityId", s2);
