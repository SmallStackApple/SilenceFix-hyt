...
IInventory {
    ...
    public void update() {
        ...
        ++this.ticksSinceSync;
        if (!this.worldObj.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + i + j2 + k2) % 200 == 0) {
            this.numPlayersUsing = 0;
            float f = 5.0f;
            for (EntityPlayer entityplayer : this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB((float)i - f, (float)j2 - f, (float)k2 - f, (float)(i + 1) + f, (float)(j2 + 1) + f, (float)(k2 + 1) + f))) {
