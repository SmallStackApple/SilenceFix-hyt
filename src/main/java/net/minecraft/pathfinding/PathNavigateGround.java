...
extends PathNavigate {
    ...
    protected boolean isDirectPathBetweenPoints(Vec3 posVec31, Vec3 posVec32, int sizeX, int sizeY, int sizeZ) {
        ...
        if (!this.isSafeToStandAt(i, (int)posVec31.yCoord, j2, sizeX += 2, sizeY, sizeZ += 2, posVec31, d0 *= d3, d1 *= d3)) {
            return false;
        }
        sizeX -= 2;
        sizeZ -= 2;
        ...
        while (k1 * k2 > 0 || l1 * l2 > 0) {
            ...
            if (this.isSafeToStandAt(i, (int)posVec31.yCoord, j2, sizeX, sizeY, sizeZ, posVec31, d0, d1)) continue;
            return false;
        }
        return true;
    }

    private boolean isSafeToStandAt(int x2, int y2, int z, int sizeX, int sizeY, int sizeZ, Vec3 vec31, double p_179683_8_, double p_179683_10_) {
        int i = x2 - sizeX / 2;
        int j2 = z - sizeZ / 2;
        if (!this.isPositionClear(i, y2, j2, sizeX, sizeY, sizeZ, vec31, p_179683_8_, p_179683_10_)) {
            return false;
        }
        for (int k2 = i; k2 < i + sizeX; ++k2) {
