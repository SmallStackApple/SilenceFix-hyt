...
public abstract class PathNavigate {
    ...
    public final PathEntity getPathToXYZ(double x2, double y2, double z) {
        return this.getPathToPos(new BlockPos(MathHelper.floor_double(x2), (int)y2, MathHelper.floor_double(z)));
    }
    ...
    public boolean tryMoveToXYZ(double x2, double y2, double z, double speedIn) {
        PathEntity pathentity = this.getPathToXYZ(MathHelper.floor_double(x2), (int)y2, MathHelper.floor_double(z));
    ...
    protected void pathFollow() {
        ...
        for (int j2 = this.currentPath.getCurrentPathIndex(); j2 < this.currentPath.getCurrentPathLength(); ++j2) {
            if (this.currentPath.getPathPointFromIndex((int)j2).yCoord == (int)vec3.yCoord) continue;
            i = j2;
            break;
        }
        float f = this.theEntity.width * this.theEntity.width * this.heightRequirement;
        for (int k2 = this.currentPath.getCurrentPathIndex(); k2 < i; ++k2) {
        ...
        for (int i1 = i - 1; i1 >= this.currentPath.getCurrentPathIndex(); --i1) {
            if (!this.isDirectPathBetweenPoints(vec3, this.currentPath.getVectorFromIndex(this.theEntity, i1), j1, k1, l2)) continue;
