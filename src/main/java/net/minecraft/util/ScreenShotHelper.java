...
public class ScreenShotHelper {
    ...
    public static IChatComponent saveScreenshot(File gameDirectory, String screenshotName, int width, int height, Framebuffer buffer) {
        try {
            ...
            boolean bl = flag = OpenGlHelper.isFramebufferEnabled() && k2 > 1;
            if (flag) {
            ...
            if (pixelBuffer == null || pixelBuffer.capacity() < l2) {
                pixelBuffer = BufferUtils.createIntBuffer((int)l2);
    ...
    public static byte[] createScreenshot(int width, int height, Framebuffer buffer) {
        try {
            Minecraft minecraft = Minecraft.getMinecraft();
            int i = Config.getGameSettings().guiScale;
            ScaledResolution scaledresolution = new ScaledResolution(minecraft);
            int j2 = scaledresolution.getScaleFactor();
            int k2 = Config.getScreenshotSize();
            boolean flag = OpenGlHelper.isFramebufferEnabled() && k2 > 1;
            if (flag) {
                Config.getGameSettings().guiScale = j2 * k2;
                ScreenShotHelper.resize(width * k2, height * k2);
                GlStateManager.pushMatrix();
    ...
    private static File getTimestampedPNGFileForDirectory(File gameDirectory) {
        ...
        int i = 1;
        File file1;
        while ((file1 = new File(gameDirectory, s2 + (i == 1 ? "" : "_" + i) + ".png")).exists()) {
            ++i;
        }
        return file1;
    }
