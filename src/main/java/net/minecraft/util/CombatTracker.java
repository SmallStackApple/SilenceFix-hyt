...
public class CombatTracker {
    ...
    private CombatEntry func_94544_f() {
        ...
        int i = 0;
        float f = 0.0f;
        for (int j2 = 0; j2 < this.combatEntries.size(); ++j2) {
            CombatEntry combatentry3;
            CombatEntry combatentry2 = this.combatEntries.get(j2);
            CombatEntry combatEntry = combatentry3 = j2 > 0 ? this.combatEntries.get(j2 - 1) : null;
            if ((combatentry2.getDamageSrc() == DamageSource.fall || combatentry2.getDamageSrc() == DamageSource.outOfWorld) && combatentry2.getDamageAmount() > 0.0f && (combatentry == null || combatentry2.getDamageAmount() > f)) {
                combatentry = j2 > 0 ? combatentry3 : combatentry2;
