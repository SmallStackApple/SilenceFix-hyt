...
extends WorldSavedData {
    ...
    private void addToNewDoorsList(BlockPos doorBlock) {
        ...
        if (i != (j2 = this.countBlocksCanSeeSky(doorBlock, enumfacing1, i + 1))) {
            this.newDoors.add(new VillageDoorInfo(doorBlock, i < j2 ? enumfacing : enumfacing1, this.tickCounter));
        }
    }
    ...
    private int countBlocksCanSeeSky(BlockPos centerPos, EnumFacing direction, int limitation) {
        ...
        for (int j2 = 1; j2 <= 5; ++j2) {
            if (!this.worldObj.canSeeSky(centerPos.offset(direction, j2)) || ++i < limitation) continue;
            return i;
        }
        return i;
    }
