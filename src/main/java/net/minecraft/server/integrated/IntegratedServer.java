...
extends MinecraftServer {
    ...
    public void saveAllWorlds(boolean dontLog) {
        if (dontLog) {
            ...
            if ((long)i < this.ticksSaveLast + (long)(j2 = this.mc.gameSettings.ofAutoSaveTicks)) {
                return;
            }
        ...
        report.getCategory().addCrashSectionCallable("Is Modded", new Callable<String>(){
            ...
            public String call() throws Exception {
                ...
                if (!s2.equals("vanilla")) {
                    return "Definitely; SilenceFix brand changed to '" + s2 + "'";
                }
    ...
    private void fixWorldTime(WorldServer p_fixWorldTime_1_) {
        ...
        if (worldinfo.getGameType().getID() == 1) {
            ...
            if (Config.isTimeDayOnly()) {
                if (j2 <= 1000L) {
                    p_fixWorldTime_1_.setWorldTime(i - j2 + 1001L);
                }
                if (j2 >= 11000L) {
                    p_fixWorldTime_1_.setWorldTime(i - j2 + 24001L);
                }
            }
            if (Config.isTimeNightOnly()) {
                if (j2 <= 14000L) {
                    p_fixWorldTime_1_.setWorldTime(i - j2 + 14001L);
                }
                if (j2 >= 22000L) {
                    p_fixWorldTime_1_.setWorldTime(i - j2 + 24000L + 14001L);
                }
            }
        }
    }
