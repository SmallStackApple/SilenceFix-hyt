...
IPlayerUsage {
    ...
    protected void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, String worldNameIn2) {
        ...
        for (int i = 0; i < this.worldServers.length; ++i) {
            ...
            if (i == 0) {
                this.worldServers[i] = this.isDemo() ? (WorldServer)new DemoWorldServer(this, isavehandler, worldinfo, j2, this.theProfiler).init() : (WorldServer)new WorldServer(this, isavehandler, worldinfo, j2, this.theProfiler).init();
                this.worldServers[i].initialize(worldsettings);
            } else {
                this.worldServers[i] = (WorldServer)new WorldServerMulti(this, isavehandler, j2, this.worldServers[0], this.theProfiler).init();
            }
        ...
        try {
            if (this.startServer()) {
                ...
                while (this.serverRunning) {
                    long k2 = MinecraftServer.getCurrentTimeMillis();
                    long j2 = k2 - this.currentTime;
                    if (j2 > 2000L && this.currentTime - this.timeOfLastWarning >= 15000L) {
                        logger.warn("Can't keep up! Did the system time change, or is the server overloaded? Running {}ms behind, skipping {} tick(s)", new Object[]{j2, j2 / 50L});
                        j2 = 2000L;
    ...
    public void updateTimeLightAndEntities() {
        ...
        for (int j2 = 0; j2 < this.worldServers.length; ++j2) {
            ...
            this.timeOfLastDimensionTick[j2][this.tickCounter % 100] = System.nanoTime() - i;
        }
        ...
        for (int k2 = 0; k2 < this.playersOnline.size(); ++k2) {
    ...
    public void addServerStatsToSnooper(PlayerUsageSnooper playerSnooper) {
        ...
        if (this.worldServers != null) {
            for (int j2 = 0; j2 < this.worldServers.length; ++j2) {
                if (this.worldServers[j2] == null) continue;
                WorldServer worldserver = this.worldServers[j2];
