...
public class PlayerManager {
    ...
    public void updatePlayerInstances() {
        ...
        if (j2 - this.previousTotalWorldTime > 8000L) {
            ...
            for (int k2 = 0; k2 < this.playerInstanceList.size(); ++k2) {
            ...
            for (int l2 = 0; l2 < this.playerInstancesToUpdate.size(); ++l2) {
    ...
    public void markBlockForUpdate(BlockPos pos) {
        int j2;
        int i = pos.getX() >> 4;
        PlayerInstance playermanager$playerinstance = this.getPlayerInstance(i, j2 = pos.getZ() >> 4, false);
    ...
    public void addPlayer(EntityPlayerMP player) {
        ...
        for (int l1 = i - this.playerViewRadius; l1 <= i + this.playerViewRadius; ++l1) {
            for (int i2 = j2 - this.playerViewRadius; i2 <= j2 + this.playerViewRadius; ++i2) {
                if (l1 >= l2 && l1 <= i1 && i2 >= j1 && i2 <= k1) {
    ...
    public void filterChunkLoadQueue(EntityPlayerMP player) {
        ...
        for (int j22 = 0; j22 < j2 * 2; ++j22) {
            chunkcoordintpair = this.getPlayerInstance(k2 + (i1 += this.xzDirectionsConst[i][0]), l2 + (j1 += this.xzDirectionsConst[i][1]), true).chunkCoords;
            if (!list.contains(chunkcoordintpair)) continue;
            player.loadedChunks.add(chunkcoordintpair);
        }
    }
    ...
    private boolean overlaps(int x1, int z1, int x2, int z2, int radius) {
        int i = x1 - x2;
        int j2 = z1 - z2;
        return i >= -radius && i <= radius ? j2 >= -radius && j2 <= radius : false;
    }
        ...
        if (d2 >= 64.0) {
            ...
            if (j1 != 0 || k1 != 0) {
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    for (int i2 = j2 - i1; i2 <= j2 + i1; ++i2) {
                        if (!this.overlaps(l1, i2, k2, l2, i1)) {
                            if (Config.isLazyChunkLoading()) {
                                set.add(new ChunkCoordIntPair(l1, i2));
                            } else {
                                this.getPlayerInstance(l1, i2, true).addPlayer(player);
                        ...
                        if (this.overlaps(l1 - j1, i2 - k1, i, j2, i1)) continue;
            ...
            if (this.numBlocksToUpdate != 0) {
                if (this.numBlocksToUpdate == 1) {
                    ...
                    for (int k2 = 0; k2 < 16; ++k2) {
                        if ((this.flagsYAreasToUpdate & 1 << k2) == 0) continue;
                        int l2 = k2 << 4;
                        List<TileEntity> list = PlayerManager.this.theWorldServer.getTileEntitiesIn(i, l2, j2, i + 16, l2 + 16, j2 + 16);
